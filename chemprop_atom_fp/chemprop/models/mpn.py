        ...
        for i, (a_start, a_size) in enumerate(a_scope):
            if a_size == 0:
                ...
                if self.fp_method == 'molecular':
                    ...
                    mol_vec_molar = torch.reshape(mol_vec_molar,(1,list(mol_vec_molar.size())[0]))
                    if self.fp_method == 'hybrid_dim0':
                        new_mol_vec = torch.cat((mol_vec,mol_vec_molar),dim=0)
                    elif self.fp_method == 'hybrid_dim1':
                        mol_vec_molar_ensemble=mol_vec_molar
                        for i in range(0,list(mol_vec.size())[0]-1):
                            mol_vec_molar_ensemble=torch.cat((mol_vec_molar_ensemble,mol_vec_molar),dim=0)
                        new_mol_vec=torch.cat((mol_vec,mol_vec_molar_ensemble),dim=1)
                        pass # TODO: modify
