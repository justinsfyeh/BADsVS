...
class MoleculeModel(nn.Module):
        ...
        if self.fp_method == 'molecular':
            ...
            if args.ffn_num_layers == 1:
                ffn = [
                    ...
                    TimeDistributed_wrapper(nn.Linear(first_linear_dim, self.output_size)) # TODO: important bias set to False !!
                ]
            else:
                ffn = [
                    TimeDistributed_wrapper(dropout),
                    TimeDistributed_wrapper(nn.Linear(first_linear_dim, args.ffn_hidden_size)) # TODO: important bias set to False !!
                ]
                for _ in range(args.ffn_num_layers - 2):
                    ffn.extend([
                        TimeDistributed_wrapper(activation),
                        TimeDistributed_wrapper(dropout),
                        TimeDistributed_wrapper(nn.Linear(args.ffn_hidden_size, args.ffn_hidden_size)), # TODO: important bias set to False !!
                    ])
                ffn.extend([
                    TimeDistributed_wrapper(activation),
                    TimeDistributed_wrapper(dropout),
                    TimeDistributed_wrapper(nn.Linear(args.ffn_hidden_size, self.output_size, bias = True)), # TODO: important bias set to False !!
            ...
            if args.ffn_num_layers == 1:
                ffn = [
                    TimeDistributed_wrapper(dropout),
                    TimeDistributed_wrapper(nn.Linear(first_linear_dim*2, self.output_size, bias = True)) # TODO: important bias set to False !!
                ]
            else:
                ffn = [
                    TimeDistributed_wrapper(dropout),
                    TimeDistributed_wrapper(nn.Linear(first_linear_dim*2, args.ffn_hidden_size, bias = True)) # TODO: important bias set to False !!
                ]
                for _ in range(args.ffn_num_layers - 2):
                    ffn.extend([
                        TimeDistributed_wrapper(activation),
                        TimeDistributed_wrapper(dropout),
                        TimeDistributed_wrapper(nn.Linear(args.ffn_hidden_size, args.ffn_hidden_size, bias = True)) # TODO: important bias set to False !!
                    ])
                ffn.extend([
                    TimeDistributed_wrapper(activation),
                    TimeDistributed_wrapper(dropout),
    ...
    def forward(self,
        ...
        """
        hidden = self.encoder(batch, features_batch, atom_descriptors_batch, atom_features_batch, bond_features_batch)
        if self.fp_method == 'molecular':
            output = self.ffn(hidden)
        else:
            output = self.ffn(hidden)
            padding_target = hidden.sum(dim=2).view(output.shape[0], -1, 1).bool().float()
